...
class Dashboard_model extends CI_Model
{
    ...
    $this->db->select('t_vechicle, t_trip_status, b.v_name, b.v_registration_no, 
                      DATE_FORMAT(t_start_date, "%d-%m-%Y %H:%i") AS t_start_date_formatted, 
                      DATE_FORMAT(t_end_date, "%d-%m-%Y %H:%i") AS t_end_date_formatted');
    $this->db->from('trips a');
    $this->db->join('vehicles b', 'a.t_vechicle = b.v_id');
    $this->db->where('`t_id` IN (SELECT MAX(`t_id`) AS `t_id` FROM trips GROUP BY `t_vechicle`)');
    $this->db->order_by('t_trip_status');
    $query = $this->db->get();
    $vechdata = $query->result_array();

    if (!empty($vechdata)) {
        foreach ($vechdata as $key => $row) {
            $vechdata[$key]['date'] = $row['t_start_date_formatted'] . " to\n" . $row['t_end_date_formatted'];
        }
        return $vechdata;
    } else {
        return '';
